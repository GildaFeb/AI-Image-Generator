...
const Nav = ({ openNav }: Props) => {
  ...
  return (
    <div
      className={`flex ${bg_style} items-center border-b-2 border-gray-800 justify-between h-[12vh] fixed z-[100] w-full px-6 md:px-16 bg-transparent`}
    >
      {/* Logo */}
      ...
      <div className="flex items-center space-x-5 md:space-x-8 text-white">
        ...
        <MenuIcon
          onClick={openNav}
          className="w-6 h-6 cursor-pointer transition-transform duration-300 hover:scale-110 md:hidden"
        />
      </div>
    </div>
  );
