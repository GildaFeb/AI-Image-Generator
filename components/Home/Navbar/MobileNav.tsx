...
const MobileNav = ({ closeNav, showNav }: Props) => {
  ...
  return (
    <div>
      ...
      <div
        className={`text-white ${navOpen} fixed justify-center flex flex-col h-screen transform transition-all
duration-1000 w-full bg-gray-950 space-y-6 z-[1050] top-0`}
      >
        ...
        <XIcon
          onClick={closeNav}
          className="absolute top-[0.7rem] right-[1.4rem] cursor-pointer sm:w-8 w-6 h-6 hover:scale-125 duration-300"
        />
      </div>
    </div>
  );
